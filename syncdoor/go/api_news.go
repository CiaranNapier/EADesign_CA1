...
func GetSyncDoor(w http.ResponseWriter, r *http.Request) {
	...
	/* 	var fetchedStrings map[string]string = make(map[string]string)
	   	for k, v := range urls {
	   		resp, err := http.Get(v)
	   		if (err != nil) {
	   			fmt.Fprintln(w, "syncdoor[ERROR]: Couldn't get " + k + " from site. " + err.Error() + "<br/>")
	   			log.Printf("syncdoor[ERROR]: Couldn't get " + k + " from site. " + err.Error())
	   		} else {
	   			if resp.StatusCode == http.StatusOK {
	   				bodyBytes, err2 := ioutil.ReadAll(resp.Body)
	   				if (err2 != nil) {
	   					fmt.Fprintln(w, "syncdoor[ERROR]: Couldn't get " + k + " from response." + err2.Error() + "<br/>")
	   					log.Printf("syncdoor[ERROR]: Couldn't get " + k + " from response." + err2.Error())
	   				} else {
	   					fetchedStrings[k] = string(bodyBytes)
	   				}
	   			} else {
	   				fmt.Fprintln(w, "syncdoor[ERROR]: HTTP returned status " + string(resp.StatusCode) + "<br/>")
	   				log.Printf("syncdoor[ERROR]: HTTP returned status " + string(resp.StatusCode))
	   			}
	   		}
	   	}
	*/
	...
	if errR != nil {
		fmt.Fprintln(w, "syncdoor[ERROR]: Can't connect to redis. "+errR.Error()+"<br>")
	}
	defer conn.Close()
	...
	if errR2 != nil {
		fmt.Fprintln(w, "syncdoor[ERROR]: EXISTS in redis failed. "+errR2.Error()+"<br>")
	} else if exists != 0 {
		// --> as the count value exists, get the count value
		errR3 := conn.Do(radix.Cmd(&theCount, "GET", "count"))
		if errR3 != nil {
			fmt.Fprintln(w, "syncdoor[ERROR]: GET in redis failed. "+errR3.Error()+"<br>")
		} else {
	...
	if respR != nil {
		fmt.Fprintln(w, "syncdoor[ERROR]: SET in redis failed. "+respR.Error()+"<br>")
	}
	...
	if templateName != "" {
		...
		fmt.Fprintln(w, "syncdoor[ERROR]: Invalid style parameter.<br>")
		log.Printf("syncdoor[ERROR]: Invalid style parameter.")
	}
}
